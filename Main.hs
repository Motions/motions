...
reprMap = [ ("PureChain", runPureChain)
  ...
  where
    runPureChain = RunRepr $ \(_ :: _ score) rs -> simulate (mkRunSettings rs :: RunSettings PureChainRepresentation score)
    runIOChain = RunRepr $ \(_ :: _ score) rs -> simulate (mkRunSettings rs :: RunSettings IOChainRepresentation score)
