...
load InitialisationSettings{..} = do
    ...
    let resRad = radius `div` resolution
        binderTypesCount = flip (-) 1 . U.length . getEnergyVector . head . head $ energyVectors
    maybeDump <- initialise initAttempts resRad bindersCount binderTypesCount energyVectors
    case maybeDump of
