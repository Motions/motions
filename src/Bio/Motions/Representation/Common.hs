...
class Monad m => SpaceLike m space where
    ...
    mmember vec space = isJust <$> mlookup vec space
    {-# INLINE mmember #-}
...
newtype WrappedHT tabletype v = WrappedHT { unwrapHT :: HT.IOHashTable tabletype Vec3 v }

instance (HTC.HashTable tabletype, MonadIO m) => SpaceLike m (WrappedHT tabletype) where
    mlookup val (WrappedHT ht) = liftIO $ HT.lookup ht val
    {-# INLINE mlookup #-}

    minsert vec val wht@(WrappedHT ht) = liftIO (HT.insert ht vec val) >> pure wht
    {-# INLINE minsert #-}

    mdelete val wht@(WrappedHT ht) = liftIO (HT.delete ht val) >> pure wht
    {-# INLINE mdelete #-}

    mfromList = fmap WrappedHT . liftIO . HT.fromList
    {-# INLINE mfromList #-}
