...
class ReadRepresentation m repr where
    ...
    getBinders' ::
        repr
        -- ^ The representation
        -> (forall c f. (MonoTraversable c, Element c ~ BinderInfo' f, Wrapper m f) => c -> m res)
    ...
    -- A simplified 'getBinders'', constraining 'f' to be 'Identity'.
    getBinders ::
        repr
    ...
    getChain' ::
        ...
        -> (forall c f. (MonoTraversable c, Element c ~ BeadInfo' f, Wrapper m f) => c -> m res)
    ...
    -- A simplified 'getChain'', constraining 'f' to be 'Identity'.
    getChain ::
        repr
        -> Int
    ...
    getAtomAt' :: Vec3 -> repr -> (forall f. Wrapper m f => Maybe (Atom' f) -> m res) -> m res
