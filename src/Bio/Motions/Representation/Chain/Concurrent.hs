...
cubeWorker Cube{..} = do
  ...
  where
    ...
    reply !move = readChan cubeChan >>= \case
        Generate -> putMVar cubeMVar move
        Perform move'@(MoveFromTo from to) -> do
            when (entersCube move') $ addAtom to
            when (leavesCube move') $ removeAtom from
            case move of
              Just m -> unless (isClose m move') $ reply move
              Nothing -> pure ()

    addAtom :: Vec3 -> IO ()
    addAtom = updateAtom S.insert
    --addAtom localBeads localBinders pos =
    --    sequence [beadIndex pos, binderIndex pos] >>= \case
    --        (Nothing, Nothing) -> error "That's bad"
    --        (Just _, Just _) -> error "That's even worse"
    --        (Just ix, _) -> modifyIORef' localBeads $ S.insert ix
    --        (_, Just ix) -> modifyIORef' localBinders $ S.insert ix

    removeAtom :: Vec3 -> IO ()
    removeAtom = updateAtom S.delete

    updateAtom :: (Int -> S.Set Int -> S.Set Int) -> Vec3 -> IO ()
    updateAtom update pos =
        sequence [beadIndex pos, binderIndex pos] >>= \case
            [Nothing, Nothing] -> error "That's bad"
            [Just _, Just _] -> error "That's even worse"
    ...
    -- TODO: slow
    beadIndex :: Vec3 -> IO (Maybe Int)
    beadIndex pos = V.findIndex ((== pos) . (^. position)) . beads <$> readIORef cubeRepr

    binderIndex :: Vec3 -> IO (Maybe Int)
    binderIndex pos = V.findIndex ((== pos) . (^. position)) . binders <$> readIORef cubeRepr
