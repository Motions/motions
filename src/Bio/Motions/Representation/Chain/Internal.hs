...
instance Monad m => ReadRepresentation m PureChainRepresentation where
    ...
    makeDump = makeDump'
    generateMove = sqrt2GenerateMove
...
sqrt2GenerateMove :: _ => ChainRepresentation f -> m (Maybe Move)
sqrt2GenerateMove repr = generateMove' repr sqrt2LegalMoves Nothing (Just $ sqrt2IllegalBeadMove repr)
{-# INLINE sqrt2GenerateMove #-}
...
generateMove' :: _ =>
  ...
  -> t1 (Move -> BinderInfo' f -> m Bool)
  -- ^A list of additional binder move constraints
  -> t2 (Move -> BeadInfo' f -> m Bool)
...
sqrt2IllegalBeadMove repr Move{..} bead = do
  ...
  where
    notOk b1 b2 = wrongQd (qd b1 b2) || sqrt2IntersectsChain (space repr) b1 b2
    wrongQd d = d <= 0 || d > 2
