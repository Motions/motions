...
gLoadDump :: _ => Dump -> m (ChainRepresentation f)
gLoadDump Dump{..} = do
...
gMakeDump :: _ => ChainRepresentation f -> m Dump
gMakeDump repr = do
...
gGenerateMove :: _ => ChainRepresentation f -> m Move
gGenerateMove repr@ChainRepresentation{..} = do
