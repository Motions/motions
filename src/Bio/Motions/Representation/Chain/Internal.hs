...
instance Monad m => Representation m PureChainRepresentation where
    type RepresentationRandomTypes m PureChainRepresentation = '[Int, Bool]
...
instance MonadIO m => Representation m IOChainRepresentation where
    type RepresentationRandomTypes m IOChainRepresentation = '[Int, Bool]
...
getRandomElement :: (MonadRandomForAll '[Int] m, DS.IsSequence s, DS.Index s ~ Int) => s -> m (Element s)
