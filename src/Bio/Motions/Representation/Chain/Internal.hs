...
type Space f = M.Map Vec3 (Located f AtomSignature)

data ChainRepresentation f = ChainRepresentation
    { space :: !(Space f)
    , binders :: !(V.Vector (Located f BinderSignature))
    , beads :: !(V.Vector (Located f BeadSignature))
...
instance Monad m => Retrievable m Identity where
    retrieve (Identity a) = pure a
    pack a = pure (Identity a)

retrieveLocated :: Retrievable m f => Located f a -> m (Located' a)
retrieveLocated (Located p a) = flip Located' a <$> retrieve p

instance (Retrievable m f, Applicative m, f ~ Identity) => ReadRepresentation m (ChainRepresentation f) where
...
instance (Retrievable m f, Applicative m, f ~ Identity) => Representation m (ChainRepresentation f) where
    ...
    performMove (MoveFromTo from to) repr
        | Binder binderSig <- atom = pure $
            let withLoc = Located' from binderSig
...
illegalBeadMove :: Retrievable m f => ChainRepresentation f -> Move -> Located f BeadSignature -> m Bool
...
getChain' :: ChainRepresentation f -> Int -> V.Vector (Located f BeadSignature)
