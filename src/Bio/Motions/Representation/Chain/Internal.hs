...
type IOChainRepresentation = ChainRepresentation IORef (WrappedHT HT.HashTable)
...
instance MonadIO m => Representation m IOChainRepresentation where
    ...
    performMove (MoveFromTo from to) repr = do
        Just atom <- mlookup from (space repr)
        space' <- mdelete from (space repr) >>= minsert to atom
