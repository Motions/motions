{- |
Module      : Bio.Motions.StateInitiation
...
initialize :: (MonadRandom m) =>
  ...
  -> m (Maybe Dump)
  -- ^Initial state
initialize maxTries r bindersCount binderTypesCount evs =
    runRepetitionGuardT maxTries $ spaceToDump r <$> initializeSpace r bindersCount binderTypesCount evs
...
spaceToDump dumpRadius space = Dump{..}
  where
    ...
    dumpChains = map (map dropIndices) $ groupWith (view beadChain) $ sortWith (view beadAtomIndex) beads
...
addLamins space positions =
  space `M.union` M.fromList [(v3, asAtom $ BinderInfo v3 $ BinderType 0) | v3 <- positions]
...
addChain chainNo indOnChain glInd start space (ev:evs) = hide $ do
    ...
    endings <-  mapM (\m -> catch $ addChain chainNo (indOnChain + 1) (glInd + 1) m newSpace evs) targets
