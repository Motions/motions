...
class (Show cb, CallbackSerialisable cb, NFData cb, Typeable cb) => Callback (mode :: Mode) cb | cb -> mode where
    ...
    runCallback :: CallbackCache m repr cache
        => cache
        -- ^The callback cache.
        -> repr
        -- ^The representation.
    ...
    updateCallback :: CallbackCache m repr cache
        => cache
        -- ^The callback cache.
        -> repr
    ...
    default updateCallback :: (CallbackCache m repr cache, mode ~ 'Post)
...
class (Monad m, ReadRepresentation m repr) => CallbackCache m repr cache where
