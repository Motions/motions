...
step = do
    ...
    prodMove <- lift2 $ getMove repr score moveProducer
    case prodMove of
...
stepAndWrite log = do
    ...
    liftIO (getNextPush outputBackend) >>= \case
      PushDump act -> do
          ...
          liftIO $ act dump cb stepCounter frameCounter score
...
simulate (RunSettings{..} :: RunSettings repr score backend producer) initialStep dump = do
    ...
    SimulationState{..} <- flip execStateT st . runExceptT $ do
        ...
        liftIO (getNextPush backend) >>= \case
            PushDump act -> liftIO $ act dump cb stepCounter 0 (score st)
            PushMove _ -> error "first frame must always be a dump"
        forever $ stepAndWrite log

    finalDump <- makeDump repr
    liftIO $ pushLastFrame finalDump stepCounter frameCounter score outputBackend
    pure finalDump
  where
    initState = do
