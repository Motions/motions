...
stepAndWrite :: (MonadRandom m, Representation m repr, Score score, MonadIO m, OutputBackend b)
...
simulate (RunSettings{..} :: RunSettings repr score) dump = do
    ...
    backend <- liftIO $ openPDBOutput outSettings dump writeIntermediatePDB simplePDB
