...
getHeader ::
    ...
    -> [String]
    -- ^Binder types descriptions
    -> [String]
    -- ^Chain names
    -> Dump
    -- ^Simulation state
    -> ProtoHeader.Header
getHeader simulationName simulationDescription binderTypesDescriptions chainNames Dump{..} =
    ProtoHeader.Header
      ...
      , binder_types_names = S.fromList $ map uFromString binderTypesDescriptions
      }
