    ...
    getMove repr score = do
        ...
        ret <- case valid of
            ...
            _ -> liftIO $ do
                ...
                pure $ Just ret
        case ret of
          Just move -> MakeMove move <$> lift (CC.updateCallback repr score move) <*> pure 1 -- TODO
          Nothing -> pure Stop
      where
        ...
        getDiffed keyframeIterator header = do
            dump' <- liftIO $ genericGet (protoGetKeyframe keyframeIterator) protoFreeKeyframe $
...
seekBinaryKF :: Int -> StateT BinaryReader IO Dump
seekBinaryKF i = do
    BinaryBackend {..} <- gets getBackend
    deltaIterator' <- liftIO $ do
