...
openBinaryInput InputSettings{..} = do
    unless (length inputFiles == 1) $ error "Specify exactly one binary input file"
    handle <- withCString (head inputFiles) protoOpenExisting
...
seekBinary BinaryBackend{..} i = do
    ...
    replicateM_ deltas
        (protoAdvanceDeltaIterator delta >> protoValidDeltaIterator kfi delta
            >>= \x -> if x == 1 then pure () else error "delta out of bounds")
