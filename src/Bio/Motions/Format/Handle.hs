...
withBinaryInput :: InputSettings -> (BinaryBackend -> Dump -> [String] -> IO a) -> IO a
withBinaryInput s f = bracket open (closeBackend . fst) (\(backend, dump) -> f backend dump (getNames backend))
    where open = openBinaryInput s >>= \backend -> (backend,) <$> seekBinaryKF backend 0
