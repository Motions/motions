...
openBinaryOutput ::
    ...
    ->  Dump
    -> IO BinaryBackend
openBinaryOutput framesPerKF OutputSettings{..} binderTypesNames dump = do
    handle <- openOutput
...
withBinaryInput :: InputSettings -> (BinaryBackend -> [String] -> IO a) -> IO a
withBinaryInput s f = bracket open closeBackend (\backend -> f backend (getNames backend))
