...
initialiseSpace r bindersCount bindersTypesCount beads = do
    ...
    addBinders stateWithChains
      where
        ...
        addBinders = foldr (<=<) pure . replicate bindersCount $ addBinder r bindersTypesCount
...
addChain :: (MonadRandom m) =>
 ...
 -> RepetitionGuardT m Space
 -- ^The state with a addeded chain
...
addChain chainNo indOnChain glInd start space (ev:evs) = hide $ do
    ...
    endings <- forM targets $
      \m -> catch $ addChain chainNo (indOnChain + 1) (glInd + 1) m newSpace evs
...
-- |Addss a binder of random type in a space
...
hookAndAddChain :: (MonadRandom m) =>
 ...
 -- ^Current space
 -> RepetitionGuardT m Space
hookAndAddChain r chainNo glInd vectors space = do
    start <- getRandomFreePosition r space
    result <- catch $ addChain chainNo 0 glInd start space vectors
    maybe (hookAndAddChain r chainNo glInd vectors space) return result
