...
applyResolution resolution x@BindingSiteInfo{..} = x {bsFrom = bsFrom `div` resolution, bsTo = bsTo `divCeil` resolution}
...
collect typesCount lengths bsInfos = map (map (EnergyVector . U.fromList)) valuesPoses
  where
    ...
    allPos = [[[(chr, pos, bsType) | bsType <- [0 .. (typesCount - 1)]] | pos <- [0 .. (chrLen -1)]] | (chr, chrLen) <- zip [0..] lengths]
