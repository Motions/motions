...
test = context "when parsing BED file" $ do
    ...
    it "should parse file without errors" $ do
        ansOK <- runEitherT $ (runStateT $ parseChrFromFile 1 "test/testdata/1.bed") chrNums
    ...
    it "should not parse file with errors" $ do
        ansErr <- runEitherT $ (runStateT $ parseChrFromFile 1 "test/testdata/wrong.bed") chrNums
