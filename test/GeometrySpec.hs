...
testIntersectsChain2 :: Spec
testIntersectsChain2 = do
    it "reports actual intersections to exist" $
        intersectsChain2 space (V3 7 8 7) (V3 7 7 8) `shouldBe` True

    it "doesn't report about non-existing intersections" $
        intersectsChain2 space (V3 7 8 7) (V3 7 8 8) `shouldBe` False
  where
...
testChainIntersectsMove = do
    it "doesn't report when move doesn't intersect with a chain" $
        assertFailure moveNotIntersecting
    --it "reports when a segment connecting the moved bead with a neibhour cuts another neighbour"
    --    assertSuccess neighbourCutsAnother
    --it "reports when a segment connecting the moved bead with a neighbour cuts a segment connecting \
    --    \ the bead with another neighbour"\
    --    assertSuccess neighbourCutsSegmentToAnother
    --it "reports when a segment connected to a neighbour of the moved bead cuts the segment connecting \
    --    \ the neighbour with the moved bead"
    --    assertSuccess neighbourOutSegmentCutsInSegment
  where
    moveNotIntersecting = ([ [ V3 0 0 0
                             , V3 0 2 0
                             , V3 1 4 0
                             , V3 (-1) 2 0
                             , V3 (-1) 1 0
    ...
    assertOutcome outcome (chainsPoss, move@Move{..}) =
        let beads = concat . dumpIndexedChains . Dump [] .
                    (map . map $ \pos -> DumpBeadInfo pos []) $ chainsPoss
            space = M.fromList [(bead ^. position, asAtom bead) | bead <- beads]
            ...
            maxQd = foldr (\con acc -> max (uncurry qd con) acc) 0 . concatMap (zip <*> tail) $ chainsPoss
...
spec :: Spec
spec = do
    context "the intersectsChain2 function"
        testIntersectsChain2
