...
testIntersectsTriangle = do
    ...
    it "reports a noncoplanar segment ending at the interior" $
  ...
  where
      ...
      assertOutcome outcome (Triangle{..}, Segment{..}) =
          forM_ (zip ([ (Triangle v1 v2 v3, Segment p q)
                      , (Triangle v1 v3 v2, Segment p q)
                      , (Triangle v1 v2 v3, Segment q p)
                      , (Triangle v1 v3 v2, Segment q p)
                      ] :: [(Triangle, Segment)]) ([1..] :: [Int]))
              $ \(config, which) -> (uncurry intersectsTriangle config, which) `shouldBe` (outcome, which)
  ...
  where
    ...
    assertOutcome outcome (s1, s2) =
        forM_ (zip ([ (s1, s2)
                    ...
                    ] :: [(Segment, Segment)]) ([1..] :: [Int]))
            $ \(config, which) -> (uncurry intersectsSegment config, which) `shouldBe` (outcome, which)
