...
testRead = do
    context "when reading correct meta data" $ do
        ...
        it "creates the correct structure" $
            eitherErrMeta `shouldBe` Right expectedMeta
    ...
    context "when reading incorrect meta data" $ do
        ...
        it "detects when mapping for lamin type is missing" $
    ...
    context "when merging incorrect or incompatible dumps" $ do
        it "detects two atoms from single dump occupying the same position" $
            mergeDumps [dumpTwoAtomsOnePos] `shouldSatisfy` isLeft
        it "detects crossings of chains from single dump" $
