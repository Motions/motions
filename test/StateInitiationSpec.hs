...
import qualified Bio.Motions.Representation.Chain.Internal as R


spec :: Spec
spec = context "when generating state" $ do
    --runIO $ print $ run (initialize 1000000 100 3 3 fewShortEVectors) 123
    ...
    it "should generate non-intersecting chains" $
        beads_positions `shouldNotSatisfy` any (\chain ->
            or $ zipWith (R.intersectsChain ans_space) chain (tail chain))
